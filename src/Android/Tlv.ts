import '../Expansion'
import {Utlis, Package, TlvPackage} from "../Utlis";
import Tea from "../Tea";

import './Struct'

import {DeviceInfo} from "./Proto/Build/DeviceInfo";
...
class Tlv {
    ...
    static Tag0106(QQ: Array<number>, QQBuff: Array<number>, PassWord: Array<number>, Time: Array<number>, TgKey: Array<number>, Guid: Array<number>, Appid: Array<number>, SubAppid: Array<number>): Array<number> {
        ...
        Pack.SetHex(Utlis.MD5HexArray(PassWord))
        Pack.SetHex(TgKey)
        Pack.SetHex([0, 0, 0, 0])
        ...
        Pack.SetHex(Utlis.MD5HexArray(Guid))
        ...
        let PassWordMd5Hex = Utlis.MD5HexArray(PassWord)

        PassWordMd5Hex = PassWordMd5Hex.concat([0, 0, 0, 0]).concat(QQ)

        let EncryptKey = Utlis.MD5HexArray(PassWordMd5Hex)
    ...
    static Tag052D(Guid: Array<number>, AndroidId: Array<number>, Fingerprint: Array<number>, InnerVersion: Array<number>, Incremental: Array<number>, CodeName: Array<number>, ProcVersion: Array<number>, Bootloader: Array<number>, BaseBand: Array<number>): Array<number> {
        let TlvPack = new TlvPackage()
        TlvPack.SetTag([5, 45])
        let DeviceParams = DeviceInfo.Struct.encode({
    ...
    static Tag0124(AndroidVersion: Array<number>): Array<number> {
        ...
        TlvPack.SetValue([
            0, 7, //length
            97, 110, 100, 114, 111, 105, 100,// type: android
    ...
    static Tag0128(GUID: Array<number>, Brand: Array<number>, Model: Array<number>) {
        let TlvPack = new TlvPackage()
        TlvPack.SetTag([1, 40])
        let Pack = new Package()
    ...
    static Tag0145(GUID: Array<number>): Array<number> {
        ...
        TlvPack.SetValue(
            Utlis.MD5HexArray(GUID)
        )
        return TlvPack.Generate()
    }

    static Tag0147(AppClientVersion: Array<number>, AppSig: Array<number>) {
        let TlvPack = new TlvPackage()
        ...
        TlvPack.SetValue([
            0, 0, 0, 16, //appid
            0, 5,
            ...AppClientVersion, //appversion 8.3.0
            0, 16,
            ...AppSig//apksign
        ])
    ...
    static Tag0154(SSOVersion: Array<number> = [0, 0, 39, 16]) {
        let TlvPack = new TlvPackage()
        TlvPack.SetTag([1, 84])
    ...
    static Tag0551(): Array<number> {
        ...
        Domain.map((value, index) => {
            Pack.SetHex([1, 0])//分隔
            let DomainBuff: Array<number> = Buffer.from(value).toJSON().data
    ...
    static Tag0187(Mac: Array<number>): Array<number> {
        ...
        TlvPack.SetValue(Utlis.MD5HexArray(Mac))
        return TlvPack.Generate()
    }
    ...
    static Tag0188(AndroidID: Array<number>): Array<number> {
        ...
        TlvPack.SetValue(Utlis.MD5HexArray(AndroidID))
        return TlvPack.Generate()
    }
    ...
    static Tag0194(IMSI: Array<number>): Array<number> {
        ...
        TlvPack.SetValue(Utlis.MD5HexArray(IMSI))
        return TlvPack.Generate()
    }
    ...
    static Tag0202(WifiSSID: Array<number>, WifiName: Array<number>): Array<number> {
        ...
        TlvPack.SetTag([2, 2])
        let Pack = new Package()
        let SSIDMd5 = Utlis.MD5HexArray(WifiSSID)
    ...
    static Tag0545(IMEI: Array<number>): Array<number> {
        let TlvPack = new TlvPackage()
        TlvPack.SetTag([5, 69])
        TlvPack.SetValue(Utlis.MD5HexArray(IMEI))
        return TlvPack.Generate()
    }
    ...
    static Tag0522Parser(Data: Array<number>) {
        Data.splice(0, 4)//???
        let QQ = Data.splice(0, 4).HexNumberToNumber()
        let CurrentIp = Data.splice(0, 4)
        let Time = Data.splice(0, 4)
        Data.splice(0, 2)//端口数据长度
        let Port = Data //是端口 具体不知道谁的
    }

    //0522 0537基本一样 无视

    static Tag0550Parser(Data: Array<number>) {
        //不知道代表什么
    }

    //cookie有效时间?
    static Tag0138(Data: Array<number>) {
